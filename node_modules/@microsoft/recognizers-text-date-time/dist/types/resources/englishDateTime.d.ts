export declare namespace EnglishDateTime {
    ...
    const YearNumRegex = "\\b(?<![$])(?<year>((1[5-9]|20)\\d{2})|2100)(?!\\.0\\b)\\b";
    ...
    const SinceRegex = "(\\b(since|after\\s+or\\s+equal\\s+to|starting\\s+(from|on|with)|as\\s+early\\s+as)\\b\\s*)|(?<!\\w|<)(>=)";
    ...
    const DateAfterRegex = "\\b((or|and)\\s+(above|after|later|greater))\\b";
